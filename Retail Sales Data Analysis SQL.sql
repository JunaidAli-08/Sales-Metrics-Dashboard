...
DROP database if exists RetailSalesDataDB;
...
LOAD DATA INFILE 'D:/data/Retail_Data_Transactions.csv'
INTO TABLE Sales_Data_Transactions
FIELDS TERMINATED BY ','
LINES TERMINATED BY '\n'
IGNORE 1 ROWS;

LOAD DATA INFILE 'D:/data/Retail_Data_Response.csv'
...
select * from sales_data_transactions group by customer_id, trans_date, tran_amount order by tran_amount desc limit 10;
...
-- Total response
SELECT sal_tran.trans_date, SUM(sal_tran.tran_amount) AS total_amount
FROM sales_data_transactions AS sal_tran
GROUP BY sal_tran.trans_date
ORDER BY sal_tran.trans_date;
...
SELECT MAX(sal_tran.trans_date) AS max_trans_date, SUM(sal_tran.tran_amount) AS total_amount
FROM sales_data_transactions AS sal_tran
GROUP BY customer_id
ORDER BY max_trans_date;
